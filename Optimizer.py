...
class Portfolio:
    def __init__(self, df, parameter):
        ...
        mapping = {
            k: v for k, v in parameter.alias.to_dict().items() if not pd.isnull(v)
        }
        ...
        self.scores_adj = (
            ...
            - df.scores.min()
            + (df.weights_ystd > 0)
            * (tcost_base + tcost_factor * tcost_enable * df.tcost)
        ).to_dict()
        ...
        for var in self.descrete:
            setattr(
                self,
                "{}_weights".format(var),
                df.groupby(var).weights_ystd.sum().sort_values(ascending=False),
            )
            setattr(self, "{}_list".format(var), df[var].unique())
            setattr(
                self, "{}_constraint".format(var), eval(parameter.loc[var, "content"])
            )
        ...
        for var in self.to_rank:
            setattr(
                self, var, getattr(self, var).rank() / getattr(self, var).rank().max()
            )
...
class Solver:
    def __init__(self, portfolio):
        ...
        self.holdStatus = LpVariable.dicts(
            "Status", self.portfolio.stock_pool, lowBound=0, upBound=1, cat=LpInteger
        )
        self.weights_raw = {
        ...
        self.trim_tol, self.trim_enable = (
            self.para.loc["trim", "val"],
            self.para.loc["trim", "enable"],
        )
    ...
    def add_obj_func(self):
        self.solver += (
            lpSum(
                [
                    self.holdStatus[s]
                    * self.portfolio.weights_tdy[s]
                    * self.portfolio.scores_adj[s]
                ...
                for sc in getattr(self.portfolio, "{}_list".format(var)):
                    self.solver += (
                        ...
                        <= (
                            ...
                            + getattr(self.portfolio, "{}_tol".format(var))
                        )
                        * lpSum(
                            [
    ...
    def weights_normalize(self):
        weights_raw = {
            int(v.name.split("_")[1]): self.portfolio.weights_tdy.get(
                int(v.name.split("_")[1]), 0
            )
        ...
        while len(fmax) or len(fmin):
            ...
            normal = {
                k: v for k, v in w.items() if k not in reach_max and k not in reach_min
            }
            normal = {
                k: (1 - sum(reach_min.values()) - sum(reach_max.values()))
                * v
                / sum(normal.values())
                for k, v in w.items()
                if k not in reach_max and k not in reach_min
            }
    ...
    def trim(self):
        ...
        for s in self.hold_weights:
            if s in self.reach_max:
                ...
                abs(self.hold_weights[s] - self.portfolio.weights_ystd[s])
                < self.trim_tol
            ):
    ...
    def score_rank(self, score):
        return sum([1 for s in self.portfolio.scores.values() if s > score]) / len(
            self.portfolio.scores.values()
        )
    ...
    def evaluate(self, save_as=None):
        ...
        ystd_score = sum(
            [
                self.portfolio.scores.get(s, 0) * self.portfolio.weights_ystd.get(s, 0)
                for s in self.portfolio.stock_pool
            ]
        )
        ...
        adjust = [
            s
            for s in adjust
            if abs(self.portfolio.weights_ystd.get(s, 0) - self.hold_weights.get(s, 0))
            > 1e-6
        ]

        print(
            "\nIn:{}\t Out:{}\t Adjust:{}\t".format(
                len(stock_in), len(stock_out), len(adjust)
            )
        )
        print(
            "\nIn:{}\t Out:{}\t Adjust:{}\t".format(
        ...
        adj_tvr = sum(
            [
                max(
                    self.hold_weights.get(s, 0) - self.portfolio.weights_ystd.get(s, 0),
                    0,
                )
                for s in adjust
            ]
        ...
        for var in self.portfolio.continuous:
            ...
            print(
                "\n>> {}:\n Before {:7.4f}   After {:.4f}   Target {:.4f}   Δ {:7.4f} ".format(
                    var,
                    ystd_values[var],
                    tdy_values[var],
                    getattr(self.portfolio, "{}_constraint".format(var)),
        ...
        self.result = pd.Series(
            {s: self.hold_weights.get(s, 0) for s in tdy_holding}, name="weights"
        )

        self.result.index.name = "Symbol"

        if save_as:
            self.result.sort_index().to_csv(
                weights_save_path + "{}.csv".format(save_as)
            )
